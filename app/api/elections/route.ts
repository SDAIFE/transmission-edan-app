...
import { createElectionSchema } from '@/lib/validations/elections.schema';
...
/**
 * Route API pour récupérer toutes les élections
 */
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${API_URL}/elections?${params.toString()}`, {
      method: 'GET',
      headers: {
    ...
    console.error('Erreur GET élections:', error);
    return NextResponse.json(
...
/**
 * Route API pour créer une élection
 */
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('Erreur POST élection:', error);
    return NextResponse.json(
      { error: getErrorMessage(error) },
      { status: 500 }
    );
