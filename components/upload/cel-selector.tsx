...
export function CelSelector({
  ...
  const loadCels = useCallback(async () => {
    try {
      setLoading(true);
      setError(null);
      ...
      if (user?.role?.code === "USER") {
        ...
        if (user.cellules && user.cellules.length > 0) {
          ...
          filteredCels = celsList.filter((cel) =>
      ...
      console.error(
        "❌ [CelSelector] Erreur lors du chargement des CELs:",
        err
      );
  ...
  const handleCelChange = (selected: string[]) => {
    if (selected.length > 0) {
      ...
      const selectedCelData = cels.find(
        (cel) => cel.codeCellule === selectedCode
      );
      if (selectedCelData) {
    ...
    <div className="space-y-2">
            ...
            <div className="text-xs text-muted-foreground mt-1">
              {cels.length > 0 ? (
                <span className="text-blue-600">
                  📋 {cels.length} CEL{cels.length > 1 ? "s" : ""} attribuée
                  {cels.length > 1 ? "s" : ""} à votre compte
                </span>
              ) : (
                <span className="text-orange-600">
                  ⚠️ Aucune CEL n&apos;est attribuée à votre compte. Contactez
      ...
      {selectedCel && (
        <div className="text-2xl text-green-600 flex justify-center items-center gap-1">
          CEL sélectionnée :{" "}
          <span className="font-medium">{selectedCel.libelleCellule}</span>
        </div>
      )}
    </div>
  );
