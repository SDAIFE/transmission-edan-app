...
import type { 
  NationalResults, 
  NationalResultsResponse,
  NationalCandidate,
  NationalResultsUtils 
} from '@/types/national-results';
...
export function NationalResultsModal({ isOpen, onClose }: NationalResultsModalProps) {
  const [nationalResults, setNationalResults] = useState<NationalResultsResponse | null>(null);
  ...
  const loadNationalResults = async () => {
    try {
      setLoading(true);
      setError(null);
      const response: NationalResultsResponse = await publicationsApi.getNationalResults();
      ...
      setError(err.message || 'Erreur lors du chargement des résultats nationaux');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handlePrint = async () => {
    ...
    try {
      ...
      doc.setFontSize(14);
      doc.text(formatNumberForPDF(nationalResults.nombreBureauxVote || 0), 18, startY + 20);
      ...
      doc.setFontSize(14);
      doc.text(formatNumberForPDF(nationalResults.inscrits || 0), 18 + cardSpacing, startY + 18);
      ...
      doc.setFontSize(14);
      doc.text(formatNumberForPDF(nationalResults.votants || 0), 18 + (cardSpacing * 2), startY + 18);
      ...
      doc.setFontSize(14);
      doc.text(`${formatPercentage(nationalResults.tauxParticipation || 0)}`, 18 + (cardSpacing * 3), startY + 20);
      ...
      doc.setFontSize(14);
      doc.text(formatNumberForPDF(nationalResults.bulletinsNuls?.nombre || 0), 18, startY + 20);
      ...
      doc.setFontSize(14);
      doc.text(formatNumberForPDF(nationalResults.suffrageExprime || 0), 18 + cardSpacing, startY + 20);
      ...
      doc.setFontSize(14);
      doc.text(formatNumberForPDF(nationalResults.bulletinsBlancs?.nombre || 0), 18 + (cardSpacing * 2), startY + 20);
      ...
      const candidatesData = nationalResults.candidats?.map(candidate => [
        candidate.parti || 'N/A',
        candidate.nom || 'N/A',
      ...
      autoTable(doc, {
        ...
        head: [
          [
            { content: 'PARTI', styles: { halign: 'center', fillColor: [111, 221, 111] } },
            { content: 'CANDIDAT', styles: { halign: 'center', fillColor: [111, 221, 111] } },
            { content: 'SCORE (CHIFFRES)', styles: { halign: 'center', fillColor: [111, 221, 111] } },
            { content: 'SCORE (LETTRES)', styles: { halign: 'center', fillColor: [111, 221, 111] } },
            { content: 'POURCENTAGE', styles: { halign: 'center', fillColor: [111, 221, 111] } }
          ]
        ],
        body: candidatesData,
        theme: 'grid',
      ...
      // Sauvegarder le PDF
      doc.save(`RESULTATS_NATIONAUX_${new Date().toISOString().split('T')[0]}.pdf`);
          ...
          <div className="flex items-center justify-between">
            <div>
              <DialogTitle className="flex items-center gap-2 text-xl">
                ...
                <span className="uppercase font-light">Résultats Nationaux</span>
              </DialogTitle>
        ...
        <div className="space-y-4 overflow-y-auto h-full p-6">
          {loading && (
            <div className="flex items-center justify-center py-8">
              ...
              <span className="ml-2">Chargement des résultats nationaux...</span>
            </div>
          )}
          ...
          {nationalResults && !loading && (
            <div ref={printRef} className="national-results-print">
                    ...
                    <table className="national-results-table stats-table">
                      ...
                      <tbody>
                        <tr className="stats-row-green">
                          <td><strong>Nombre de bureaux de vote</strong></td>
                          <td>{formatNumber(nationalResults.nombreBureauxVote || 0)}</td>
                          <td>{numberToWords(nationalResults.nombreBureauxVote || 0)}</td>
                        </tr>
                        <tr className="stats-row-green">
                          <td><strong>Inscrits</strong></td>
                          <td>{formatNumber(nationalResults.inscrits || 0)}</td>
                          <td>{numberToWords(nationalResults.inscrits || 0)}</td>
                        </tr>
                        <tr className="stats-row-green">
                          <td><strong>Votants</strong></td>
                        ...
                        <tr className="stats-row-white">
                          ...
                          <td>{formatPercentage(nationalResults.tauxParticipation || 0)}</td>
                          <td></td>
                        </tr>
                        <tr className="stats-row-white">
                          <td><strong>Bulletins nuls</strong></td>
                          <td>{formatNumber(nationalResults.bulletinsNuls?.nombre || 0)} ({formatPercentage(nationalResults.bulletinsNuls?.pourcentage || 0)})</td>
                          <td>{numberToWords(nationalResults.bulletinsNuls?.nombre || 0)}</td>
                        </tr>
                        <tr className="stats-row-green">
                          <td><strong>Suffrages exprimés</strong></td>
                          <td>{formatNumber(nationalResults.suffrageExprime || 0)}</td>
                          <td>{numberToWords(nationalResults.suffrageExprime || 0)}</td>
                        </tr>
                      </tbody>
                    </table>
                  </div>
              ...
              {nationalResults.candidats && nationalResults.candidats.length > 0 && (
                <div className="mb-8 results-section">
                  ...
                  <div>
                    <table className="national-results-table candidates-table">
                      ...
                      <tbody>
                        {nationalResults.candidats.map((candidate, index) => {
                          return (
                            <tr key={candidate.numeroOrdre || index} className="candidate-row">
                              <td>
                                <span className="party-badge">{candidate.parti || 'N/A'}</span>
                              </td>
                              <td>
                                <div className="candidate-info">
                                  ...
                                  <span className="candidate-name">{candidate.nom || 'N/A'}</span>
                                </div>
                              </td>
                              <td>{formatNumber(candidate.score || 0)}</td>
                              <td>{numberToWords(candidate.score || 0)}</td>
                              <td>{formatPercentage(candidate.pourcentage || 0)}</td>
                            </tr>
                          );
                        })}
                        ...
                        {nationalResults.bulletinsBlancs && (
                          <tr className="bulletins-blancs-row">
                            <td colSpan={2}>
                              <span className="bulletins-blancs-label">BULLETINS BLANCS</span>
                            </td>
                            <td>{formatNumber(nationalResults.bulletinsBlancs.nombre || 0)}</td>
                            <td>{numberToWords(nationalResults.bulletinsBlancs.nombre || 0)}</td>
                            <td>{formatPercentage(nationalResults.bulletinsBlancs.pourcentage || 0)}</td>
                          </tr>
                        )}
                      </tbody>
                    </table>
        ...
        <div className="flex justify-between items-center pt-4 border-t bg-gray-50 px-6 py-4 no-print">
          <div className="text-sm text-gray-600">
            ...
            {nationalResults?.candidats && (
              <span className="ml-4">
                <span className="font-medium">Candidats :</span> {nationalResults.candidats.length}
              </span>
            )}
          </div>
