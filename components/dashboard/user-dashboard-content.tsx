...
import { DashboardDebug } from './dashboard-debug';
...
export function UserDashboardContent({ user }: UserDashboardContentProps) {
  ...
  const userStats = userMetrics ? [
    { 
      ...
      value: userMetrics.totalCels, 
      icon: Building2, 
      color: 'text-primary' 
    },
    { 
      label: 'CELs avec Import', 
      value: userMetrics.celsAvecImport, 
      icon: CheckCircle, 
      color: 'text-green-600', 
      subtitle: `Taux: ${userMetrics.tauxProgression.toFixed(1)}%` 
    },
    { 
      label: 'CELs sans Import', 
      value: userMetrics.celsSansImport, 
      icon: Clock, 
      color: 'text-yellow-600' 
    },
    { 
      label: 'CELs Assignées', 
      value: userMetrics.celsAssignees, 
      icon: FileText, 
      color: 'text-purple-600',
      subtitle: `Progression: ${userMetrics.tauxProgressionPersonnel.toFixed(1)}%` 
    }
  ] : [];
