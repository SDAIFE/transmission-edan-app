...
export function RealtimeMetrics({ 
  ...
  const fetchMetrics = async () => {
    try {
      ...
      console.warn('⚠️ [RealtimeMetrics] API non disponible, utilisation des données mockées');
      console.error('❌ [RealtimeMetrics] Erreur API:', err);
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl text-primary font-bold">Métriques Temps Réel</h2>
      ...
      <div className="grid gap-4 md:grid-cols-4">
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground">
              Taux: {metrics.tauxProgression.toFixed(1)}%
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-yellow-600">{metrics.celsSansImport}</div>
            <p className="text-xs text-muted-foreground">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-red-600">{metrics.nombreErreurs}</div>
            <p className="text-xs text-muted-foreground">
          ...
          <CardContent>
            <div className="grid gap-4 md:grid-cols-4">
              <div className="flex items-center space-x-2">
                ...
                <Badge variant="outline">{metrics.departements.totalDepartements}</Badge>
              </div>
              <div className="flex items-center space-x-2">
                <CheckCircle className="h-4 w-4 text-green-600" />
        ...
        <CardContent>
          <div className="grid gap-4 md:grid-cols-3">
            <div className="flex items-center space-x-2">
              ...
              <Badge variant="secondary">{metrics.activiteRecente.importsAujourdhui}</Badge>
            </div>
            <div className="flex items-center space-x-2">
              <Users className="h-4 w-4 text-blue-600" />
              <span className="text-sm font-medium">Publications aujourd'hui</span>
              <Badge variant="secondary">{metrics.activiteRecente.publicationsAujourdhui}</Badge>
            </div>
            <div className="flex items-center space-x-2">
              <Activity className="h-4 w-4 text-purple-600" />
              <span className="text-sm font-medium">Connexions aujourd'hui</span>
              <Badge variant="secondary">{metrics.activiteRecente.connexionsAujourdhui}</Badge>
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      {/* Imports en cours */}
      {metrics.importsEnCours.nombre > 0 && (
        <Card>
          <CardHeader>
            <CardTitle>Imports en Cours</CardTitle>
            <CardDescription>
              {metrics.importsEnCours.nombre} import(s) en cours de traitement
            </CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-2">
              {metrics.importsEnCours.liste.map((importItem, index) => (
                <div key={index} className="flex items-center justify-between p-2 bg-blue-50 rounded">
                  <div>
                    <span className="font-medium">{importItem.codeCellule}</span>
                    <span className="text-sm text-muted-foreground ml-2">
                      {importItem.nomFichier}
                    </span>
                  </div>
                  <div className="text-sm text-muted-foreground">
                    {formatTimestamp(importItem.dateImport)}
                  </div>
                </div>
              ))}
            </div>
