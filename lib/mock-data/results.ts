...
export const calculateTotals = (bureaux: BureauVote[]) => {
  ...
  const tauxParticipation = totals.inscrits > 0 ? 
    Number((totals.votants / totals.inscrits * 100).toFixed(2)) : 0;
  ...
  const results: VoteResult[] = Array.from(candidateVotes.entries()).map(([candidateId, votes]) => ({
    candidateId,
    votes,
    percentage: totals.exprimes > 0 ? Number((votes / totals.exprimes * 100).toFixed(2)) : 0
  }));
...
export const generateResults = (inscrits: number, tauxParticipation: number): { 
  ...
  return { votants, votantsHommes, votantsFemmes, exprimes, blancs, nuls, results };
};
...
export const generateBureaux = (lieuId: string, count: number, candidates: Candidate[]): BureauVote[] => {
  return Array.from({ length: count }, (_, i) => {
    ...
    const results: VoteResult[] = candidates.map((candidate, index) => {
