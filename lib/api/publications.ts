...
export const publicationsApi = {
  ...
  publishDepartment: async (id: string): Promise<PublicationActionResult> => {
    try {
      const response = await apiClient.post(`/publications/departments/${id}/publish`);
      return response.data;
    } catch (error: any) {
  ...
  cancelPublication: async (id: string): Promise<PublicationActionResult> => {
    try {
      const response = await apiClient.post(`/publications/departments/${id}/cancel`);
      return response.data;
    } catch (error: any) {
  ...
  getDepartmentDetails: async (id: string): Promise<any> => {
    try {
      const response = await apiClient.get(`/publications/departments/${id}/details`);
      return response.data;
    } catch (error: any) {
  ...
  getEntities: async (query?: EntityListQuery): Promise<EntityListResponse | null> => {
    try {
      ...
      if (response.data && response.data.departments && !response.data.entities) {
        ...
        const entities = response.data.departments.map((dept: any) => ({
          id: dept.id,
  ...
  getCommuneDetails: async (id: string): Promise<CommuneDetails> => {
    try {
      const response = await apiClient.get(`/publications/communes/${id}/details`);
      return response.data;
    } catch (error: any) {
  ...
  publishCommune: async (id: string): Promise<EntityActionResult> => {
    try {
      ...
      return {
        success: false,
        message: error?.response?.data?.message || 'Erreur lors de la publication de la commune',
        error: error?.response?.data?.error
      };
    }
  },
  ...
  cancelCommune: async (id: string): Promise<EntityActionResult> => {
    try {
      ...
      return {
        success: false,
        message: error?.response?.data?.message || 'Erreur lors de l\'annulation de la commune',
        error: error?.response?.data?.error
      };
    }
  },
  ...
  getAbidjanCommunes: async (query?: Omit<EntityListQuery, 'codeDepartement'>): Promise<EntityListResponse | null> => {
    try {
      //en developpement
      if (process.env.NODE_ENV === 'development') {
  ...
  getNationalResults: async (filters?: NationalResultsFilters): Promise<NationalResultsResponse> => {
    try {
      ...
      if (error?.response?.status === 404) {
        ...
        throw new Error('La route des résultats nationaux n\'est pas encore disponible côté backend');
      }
      
      if (error?.response?.status === 403) {
        console.warn('⚠️ [PublicationsAPI] Permissions insuffisantes pour accéder aux résultats nationaux');
        throw new Error('Permissions insuffisantes pour accéder aux résultats nationaux');
      }
