...
export const legislativesPublicationsApi = {
  ...
  getStats: async (): Promise<LegislativePublicationStats | null> => {
    try {
      if (process.env.NODE_ENV === "development") {
        ...
        console.log(
          "📊 [LegislativesPublicationsAPI] Récupération des statistiques..."
        );
      }
      ...
      if (process.env.NODE_ENV === "development") {
        // eslint-disable-next-line no-console
        console.log(
          "✅ [LegislativesPublicationsAPI] Statistiques récupérées:",
          response.data
        );
      }
  ...
  getCirconscriptions: async (
    ...
    try {
      if (process.env.NODE_ENV === "development") {
        // eslint-disable-next-line no-console
        console.log(
          "📋 [LegislativesPublicationsAPI] Récupération des circonscriptions:",
          query
        );
      }
      ...
      if (process.env.NODE_ENV === "development") {
        // eslint-disable-next-line no-console
        console.log(
          "✅ [LegislativesPublicationsAPI] Circonscriptions récupérées:",
          {
  ...
  publishCirconscription: async (
    ...
    try {
      ...
      if (process.env.NODE_ENV === "development") {
        // eslint-disable-next-line no-console
        console.log(
          "✅ [LegislativesPublicationsAPI] Publication réussie:",
          response.data
        );
      }
      ...
      if (error?.response?.status === 403) {
        throw new Error(
          "Vous n'avez pas les permissions nécessaires pour publier une circonscription."
        );
      }
   ...
   * Permissions: SADMIN, ADMIN uniquement
   *
   * ⚠️ IMPORTANT: Lors de l'annulation, le backend doit mettre le champ ETA_RESULTAT_CEL
   * des CELs concernées à "I" (Importé) et non "CANCELLED" pour permettre la republication
   * de la circonscription. Si ETA_RESULTAT_CEL est mis à "CANCELLED", la circonscription
   * ne pourra pas être republiée car importedCels ne correspondra plus à totalCels.
   *
   * @param codeCirconscription Code de la circonscription (COD_CE, ex: "004")
  ...
  cancelPublication: async (
    ...
    try {
      ...
      if (process.env.NODE_ENV === "development") {
        // eslint-disable-next-line no-console
        console.log(
          "✅ [LegislativesPublicationsAPI] Annulation réussie:",
          response.data
        );
      }
      ...
      if (error?.response?.status === 403) {
        throw new Error(
          "Vous n'avez pas les permissions nécessaires pour annuler une publication."
        );
      }
  ...
  getCirconscriptionData: async (
    ...
    try {
      ...
      if (process.env.NODE_ENV === "development") {
        // eslint-disable-next-line no-console
        console.log(
          "✅ [LegislativesPublicationsAPI] Données agrégées récupérées:",
          {
            code: response.data.codeCirconscription,
  ...
  getNationalData: async (): Promise<NationalDataResponse> => {
    try {
      if (process.env.NODE_ENV === "development") {
        // eslint-disable-next-line no-console
        console.log(
          "🌍 [LegislativesPublicationsAPI] Récupération des données nationales..."
        );
      }
      ...
      if (process.env.NODE_ENV === "development") {
        // eslint-disable-next-line no-console
        console.log(
          "✅ [LegislativesPublicationsAPI] Données nationales récupérées:",
          {
            candidats: response.data.candidats.length,
